...
usage() {
  cat <<'EOF'
Usage: create-candidate-e2e-cluster.sh --runtime-dir PATH --candidate-dir PATH
...
[[ -n "$runtime_dir" && -n "$candidate_dir" && -n "$runner_image" && -n "$cluster_name" && -n "$state_dir" ]] || die "All arguments are required"
...
runtime_dir="$(resolve_directory "$runtime_dir")"; candidate_dir="$(resolve_directory "$candidate_dir")"
