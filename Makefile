...
ROOT_DIR := $(CURDIR)
BUNDLE_DIR ?= $(ROOT_DIR)/../volcano-offline-e2e-bundle
BUNDLE_INSPECTION ?= $(ROOT_DIR)/.work/offline-bundle.detected.yaml
CANDIDATE_DIR ?= $(ROOT_DIR)/../volcano-master
...
CANDIDATE_BENCHMARK_ASSET_DIR ?= $(ROOT_DIR)/.work/offline-assets/benchmark-tools/$(CANDIDATE_EXPECTED_COMMIT)
...
CANDIDATE_REGISTRY_HOST ?= localhost:15000
...
CANDIDATE_DEPS_ASSET_DIR ?= $(ROOT_DIR)/offline-assets/go-mod/$(CANDIDATE_EXPECTED_COMMIT)
...
CANDIDATE_E2E_CLUSTER_STATE ?= $(ROOT_DIR)/.work/clusters/$(CANDIDATE_E2E_CLUSTER_NAME)
CANDIDATE_E2E_REPORT_DIR ?= $(ROOT_DIR)/.work/reports/$(CANDIDATE_E2E_CLUSTER_NAME)
...
export PERFORMANCE_GUARD_TOOLS_IMAGE = $(PERFORMANCE_TOOLS_IMAGE)

.PHONY: help inspect-bundle scan-benchmark-deps candidate-preflight candidate-prepare-deps package-candidate-deps import-candidate-deps candidate-build-binaries candidate-build-audit-exporter package-performance-tools package-benchmark-assets import-benchmark-assets candidate-build-images candidate-publish-images candidate-create-cluster candidate-create-audit-cluster candidate-create-e2e-cluster candidate-deploy candidate-install-monitoring candidate-smoke candidate-community-benchmark candidate-audit-community-benchmark candidate-e2e candidate-timestamp-profile compare-baseline candidate-cleanup contract-validate mock-prepare mock-inspect mock-cleanup dry-run aggregate-demo contract-demo test
...
help:
	@printf '%s\n' \
	  ...
	  '' \
	  'Offline E2E bundle:' \
	  '  make inspect-bundle      Inspect the bundle without mutating it' \
	  ...
	  '  make candidate-create-e2e-cluster Create isolated four-worker E2E cluster' \
	  ...
	  '  make candidate-e2e E2E_SUITE=... Run an offline upstream E2E target' \
...
candidate-preflight:
	...
	bash adapters/offline-e2e-bundle/preflight-candidate.sh \
		--bundle-dir "$(BUNDLE_DIR)" \
		--candidate-dir "$(CANDIDATE_DIR)" \
...
candidate-prepare-deps:
	...
	bash adapters/offline-e2e-bundle/prepare-candidate-deps.sh \
		--bundle-dir "$(BUNDLE_DIR)" \
		--candidate-dir "$(CANDIDATE_DIR)" \
...
package-candidate-deps:
	...
	bash adapters/offline-e2e-bundle/package-candidate-deps.sh \
		--bundle-dir "$(BUNDLE_DIR)" \
		--candidate-dir "$(CANDIDATE_DIR)" \
...
import-candidate-deps:
	...
	bash adapters/offline-e2e-bundle/import-candidate-deps.sh \
		--bundle-dir "$(BUNDLE_DIR)" \
		--asset-dir "$(CANDIDATE_DEPS_ASSET_DIR)" \
		"$${runner_args[@]}" \
		"$${expected_args[@]}"
...
candidate-build-binaries:
	...
	bash adapters/offline-e2e-bundle/build-candidate-binaries.sh \
		--bundle-dir "$(BUNDLE_DIR)" \
		--candidate-dir "$(CANDIDATE_DIR)" \
		--runner-image "$(CANDIDATE_RUNNER_IMAGE)" \
...
candidate-build-audit-exporter:
	...
	bash adapters/offline-e2e-bundle/build-candidate-audit-exporter.sh \
		--bundle-dir "$(BUNDLE_DIR)" \
		--candidate-dir "$(CANDIDATE_DIR)" \
		--runner-image "$(CANDIDATE_RUNNER_IMAGE)" \
...
candidate-build-images:
	bash adapters/offline-e2e-bundle/build-candidate-images.sh \
		--bundle-dir "$(BUNDLE_DIR)" \
		--candidate-dir "$(CANDIDATE_DIR)" \
		--build-dir "$(CANDIDATE_BUILD_DIR)"

candidate-publish-images:
	bash adapters/offline-e2e-bundle/publish-candidate-images.sh \
		--candidate-dir "$(CANDIDATE_DIR)" \
		--build-dir "$(CANDIDATE_BUILD_DIR)"

candidate-create-cluster:
	@test -n "$(CANDIDATE_RUNNER_IMAGE)" || { echo 'CANDIDATE_RUNNER_IMAGE is required' >&2; exit 1; }
	bash adapters/offline-e2e-bundle/create-candidate-cluster.sh \
		--bundle-dir "$(BUNDLE_DIR)" \
		--candidate-dir "$(CANDIDATE_DIR)" \
		--runner-image "$(CANDIDATE_RUNNER_IMAGE)" \
...
candidate-create-audit-cluster:
	@test -n "$(CANDIDATE_RUNNER_IMAGE)" || { echo 'CANDIDATE_RUNNER_IMAGE is required' >&2; exit 1; }
	bash adapters/offline-e2e-bundle/create-candidate-audit-cluster.sh \
		--bundle-dir "$(BUNDLE_DIR)" \
		--candidate-dir "$(CANDIDATE_DIR)" \
		--runner-image "$(CANDIDATE_RUNNER_IMAGE)" \
...
candidate-create-e2e-cluster:
	@test -n "$(CANDIDATE_RUNNER_IMAGE)" || { echo 'CANDIDATE_RUNNER_IMAGE is required' >&2; exit 1; }
	bash adapters/offline-e2e-bundle/create-candidate-e2e-cluster.sh \
		--bundle-dir "$(BUNDLE_DIR)" \
		--candidate-dir "$(CANDIDATE_DIR)" \
		--runner-image "$(CANDIDATE_RUNNER_IMAGE)" \
		--cluster-name "$(CANDIDATE_E2E_CLUSTER_NAME)" \
		--state-dir "$(CANDIDATE_E2E_CLUSTER_STATE)"

candidate-deploy:
	bash adapters/offline-e2e-bundle/deploy-candidate.sh \
		--bundle-dir "$(BUNDLE_DIR)" \
		--candidate-dir "$(CANDIDATE_DIR)" \
		--runner-image "$(CANDIDATE_RUNNER_IMAGE)" \
		--cluster-name "$(CANDIDATE_CLUSTER_NAME)" \
...
candidate-install-monitoring:
	bash adapters/offline-e2e-bundle/install-candidate-monitoring.sh \
		--bundle-dir "$(BUNDLE_DIR)" \
		--candidate-dir "$(CANDIDATE_DIR)" \
		--runner-image "$(CANDIDATE_RUNNER_IMAGE)" \
		--state-dir "$(CANDIDATE_AUDIT_CLUSTER_STATE)" \
...
candidate-smoke:
	bash adapters/offline-e2e-bundle/verify-candidate-smoke.sh \
		--bundle-dir "$(BUNDLE_DIR)" \
		--candidate-dir "$(CANDIDATE_DIR)" \
		--runner-image "$(CANDIDATE_RUNNER_IMAGE)" \
		--cluster-name "$(CANDIDATE_CLUSTER_NAME)" \
...
candidate-community-benchmark:
	...
	bash adapters/offline-e2e-bundle/run-community-benchmark.sh \
		--bundle-dir "$(BUNDLE_DIR)" \
		--candidate-dir "$(CANDIDATE_DIR)" \
		--runner-image "$(CANDIDATE_RUNNER_IMAGE)" \
		--state-dir "$(CANDIDATE_CLUSTER_STATE)" \
...
candidate-e2e:
	bash adapters/offline-e2e-bundle/run-candidate-e2e.sh \
		--bundle-dir "$(BUNDLE_DIR)" \
		--candidate-dir "$(CANDIDATE_DIR)" \
		--runner-image "$(CANDIDATE_RUNNER_IMAGE)" \
		--cluster-name "$(CANDIDATE_E2E_CLUSTER_NAME)" \
		--state-dir "$(CANDIDATE_E2E_CLUSTER_STATE)" \
		--output-dir "$(CANDIDATE_E2E_REPORT_DIR)" \
		--asset-dir "$(CANDIDATE_BENCHMARK_ASSET_DIR)" \
		--suite "$(E2E_SUITE)"
...
candidate-cleanup:
	bash adapters/offline-e2e-bundle/cleanup-candidate-cluster.sh \
		--bundle-dir "$(BUNDLE_DIR)" \
		--candidate-dir "$(CANDIDATE_DIR)" \
		--runner-image "$(CANDIDATE_RUNNER_IMAGE)" \
		--cluster-name "$(CANDIDATE_CLUSTER_NAME)" \
