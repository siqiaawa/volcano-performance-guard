...
INSPECT = ROOT / "adapters" / "offline-e2e-bundle" / "inspect.sh"
RUN_CANDIDATE = ROOT / "adapters" / "offline-e2e-bundle" / "run-candidate.sh"
PREFLIGHT_CANDIDATE = ROOT / "adapters" / "offline-e2e-bundle" / "preflight-candidate.sh"
DEPLOY_CANDIDATE = ROOT / "adapters" / "offline-e2e-bundle" / "deploy-candidate.sh"
SCAN_BENCHMARK_DEPS = ROOT / "scripts" / "scan-benchmark-deps.py"
TIMESTAMP_BENCHMARK = ROOT / "adapters" / "offline-e2e-bundle" / "run-timestamp-benchmark.py"
BUILD_AUDIT_EXPORTER = ROOT / "adapters" / "offline-e2e-bundle" / "build-candidate-audit-exporter.sh"
RUN_COMMUNITY_BENCHMARK = ROOT / "adapters" / "offline-e2e-bundle" / "run-community-benchmark.sh"
CREATE_AUDIT_CLUSTER = ROOT / "adapters" / "offline-e2e-bundle" / "create-candidate-audit-cluster.sh"
INSTALL_MONITORING = ROOT / "adapters" / "offline-e2e-bundle" / "install-candidate-monitoring.sh"
PREPARE_CANDIDATE_DEPS = ROOT / "adapters" / "offline-e2e-bundle" / "prepare-candidate-deps.sh"
...
class OfflineBundleAdapterTests(unittest.TestCase):
    ...
    def test_missing_bundle_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            result = subprocess.run(
                ["bash", str(INSPECT), "--bundle-dir", str(missing)],
                check=False,
                capture_output=True,
                text=True,
            )
    ...
    def test_import_writes_shell_safe_registry_environment_keys(self) -> None:
        key = IMPORT_MODULE.environment_key(
            "localhost:15000/volcanosh/performance-guard-tools:deadbeef"
        )
