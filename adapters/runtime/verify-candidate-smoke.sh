...
usage() {
  cat <<'EOF'
Usage: verify-candidate-smoke.sh --runtime-dir PATH --candidate-dir PATH
...
runtime_dir=""; candidate_dir=""; runner_image=""; cluster_name=""; state_dir=""; report_dir=""
while [[ $# -gt 0 ]]; do
  case "$1" in
...
[[ -n "$runtime_dir" && -n "$candidate_dir" && -n "$runner_image" && -n "$cluster_name" && -n "$state_dir" && -n "$report_dir" ]] || die "All arguments are required"
