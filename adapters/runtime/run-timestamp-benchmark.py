...
def provenance(arguments: argparse.Namespace, profile: dict[str, Any], node: dict[str, Any], scheduler_config: dict[str, Any]) -> tuple[str, dict[str, Any]]:
    runtime_detection = yaml.safe_load((ROOT / "configs" / "runtime.detected.yaml").read_text(encoding="utf-8"))
    bundle = runtime_detection["bundle"]
    ...
    record = {
        ...
        "runner": runner,
        "stableVolcanoVersion": f"reference-commit-{runtime_detection['referenceVolcano']['commit']}",
...
def main() -> int:
    ...
    try:
        for path in (args.runtime_dir, args.candidate_dir, args.state_dir):
            if not path.is_dir():
        ...
        subject_version = args.subject_version or candidate_commit
