...
usage() {
  cat <<'EOF'
Usage: build-candidate-audit-exporter.sh --runtime-dir PATH --candidate-dir PATH
...
[[ -n "$runtime_dir" && -n "$candidate_dir" && -n "$runner_image" && -n "$output_dir" ]] || \
  die "--runtime-dir, --candidate-dir, --runner-image, and --output-dir are required"
