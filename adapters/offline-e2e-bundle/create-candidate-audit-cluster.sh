...
usage() {
  cat <<'EOF'
Usage: create-candidate-audit-cluster.sh --bundle-dir PATH --candidate-dir PATH
...
bundle_dir=""; candidate_dir=""; runner_image=""; cluster_name=""; state_dir=""; asset_dir=""
...
[[ -n "$bundle_dir" && -n "$candidate_dir" && -n "$runner_image" && -n "$cluster_name" && -n "$state_dir" && -n "$asset_dir" ]] || die "All arguments are required"
