...
usage() {
  cat <<'EOF'
Usage: build-candidate-binaries.sh --bundle-dir PATH --candidate-dir PATH
                                   --runner-image NAME --output-dir PATH
                                   [--expected-commit SHA]

Build candidate component binaries in the offline Runner with network mode
none. Candidate source is read-only and all outputs are external.
EOF
}
