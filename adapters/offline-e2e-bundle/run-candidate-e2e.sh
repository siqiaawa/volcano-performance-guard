...
usage() {
  cat <<'EOF'
Usage: run-candidate-e2e.sh --bundle-dir PATH --candidate-dir PATH
...
bundle_dir=""; candidate_dir=""; runner_image=""; cluster_name=""; state_dir=""; output_dir=""; asset_dir=""; suite="SCHEDULINGBASE"
while [[ $# -gt 0 ]]; do
  case "$1" in
...
[[ -n "$bundle_dir" && -n "$candidate_dir" && -n "$runner_image" && -n "$cluster_name" && -n "$state_dir" && -n "$output_dir" ]] || die "All arguments are required"
...
bundle_dir="$(resolve_directory "$bundle_dir")"; candidate_dir="$(resolve_directory "$candidate_dir")"; state_dir="$(resolve_directory "$state_dir")"
...
run_candidate=("$script_dir/run-candidate.sh" --bundle-dir "$bundle_dir" --candidate-dir "$candidate_dir" --runner-image "$runner_image" --state-dir "$state_dir" --output-dir "$output_dir" --network host --with-docker-socket --)
