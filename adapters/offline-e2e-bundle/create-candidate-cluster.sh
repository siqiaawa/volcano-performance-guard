...
usage() {
  cat <<'EOF'
Usage: create-candidate-cluster.sh --bundle-dir PATH --candidate-dir PATH
                                   --runner-image NAME --cluster-name NAME
                                   --state-dir PATH

Create a dedicated one-node Kind cluster, configure the bundle registry
...
[[ -n "$bundle_dir" && -n "$candidate_dir" && -n "$runner_image" && -n "$cluster_name" && -n "$state_dir" ]] || \
  die "--bundle-dir, --candidate-dir, --runner-image, --cluster-name, and --state-dir are required"
