...
usage() {
  cat <<'EOF'
Usage: prepare-candidate-deps.sh --bundle-dir PATH --candidate-dir PATH
                                  [--expected-commit SHA]
...
Discover missing Go modules with the bundle Runner, optionally package only
those modules during an explicitly online phase, import the supplement into a
derived candidate Runner, and run the offline preflight again. The bundle and
...
base_runner="$(awk -F= '$1 == "RUNNER_IMAGE" {sub(/^[^=]*=/, ""); print; found=1} END {if (!found) exit 1}' \
  "$bundle_dir/offline.env" | tr -d '\r')" || die "Bundle offline.env is missing RUNNER_IMAGE"
[[ "$base_runner" =~ ^[A-Za-z0-9_./:@-]+$ ]] || die "Bundle RUNNER_IMAGE is unsafe"
...
if [[ -z "$asset_dir" ]]; then
  asset_dir="$script_dir/../../offline-assets/go-mod/$candidate_commit"
fi
...
log_info "Discovering missing Go modules with bundle Runner: $base_runner"
set +e
...
log_warn "Bundle Runner is missing $missing_count candidate Go module(s)"
