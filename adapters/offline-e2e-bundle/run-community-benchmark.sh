...
usage() {
  cat <<'EOF'
Usage: run-community-benchmark.sh --bundle-dir PATH --candidate-dir PATH
...
[[ -n "$bundle_dir" && -n "$candidate_dir" && -n "$runner_image" && -n "$state_dir" && -n "$output_dir" && -n "$cluster_name" ]] || \
  die "All bundle, candidate, Runner, state, output, and cluster arguments are required"
