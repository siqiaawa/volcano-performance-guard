...
usage() {
  cat <<'EOF'
Usage: install-candidate-monitoring.sh --bundle-dir PATH --candidate-dir PATH
                                      --runner-image NAME --state-dir PATH
                                      --asset-dir PATH --report-dir PATH
EOF
}

bundle_dir=""; candidate_dir=""; runner_image=""; state_dir=""; asset_dir=""; report_dir=""
while [[ $# -gt 0 ]]; do
  case "$1" in
...
[[ -n "$bundle_dir" && -n "$candidate_dir" && -n "$runner_image" && -n "$state_dir" && -n "$asset_dir" && -n "$report_dir" ]] || die "All arguments are required"
require_cmd git docker
bundle_dir="$(resolve_directory "$bundle_dir")"; candidate_dir="$(resolve_directory "$candidate_dir")"; state_dir="$(resolve_directory "$state_dir")"; asset_dir="$(resolve_directory "$asset_dir")"
...
run_candidate=("$script_dir/run-candidate.sh" --bundle-dir "$bundle_dir" --candidate-dir "$candidate_dir" --runner-image "$runner_image" --state-dir "$state_dir" --output-dir "$report_dir" --network host --with-docker-socket --)
