...
contract. Go module downloads are the only network access allowed during a
version build. Run ./setup.sh once to prepare a fresh clone; fixed-compare uses
stable/volcano by default.
EOF
}
...
run_version() {
  ...
  if [[ "$commit" != "$reference_commit" ]]; then
    log_info "Building $label with online Go dependency mode through $goproxy"
    (
      export PERFORMANCE_GUARD_GO_MODE=online
      export PERFORMANCE_GUARD_NETWORK_MODE=host
      export PERFORMANCE_GUARD_GO_PROXY="$goproxy"
      run_make candidate-build-binaries \
        RUNTIME_DIR="$runtime_dir" CANDIDATE_DIR="$version_dir" \
        CANDIDATE_EXPECTED_COMMIT="$commit" CANDIDATE_RUNNER_IMAGE="$runner_image" \
  ...
  active_runner_image="$runner_image"
  local status=0
  set +e
  run_make candidate-create-cluster \
    RUNTIME_DIR="$runtime_dir" CANDIDATE_DIR="$version_dir" \
    CANDIDATE_RUNNER_IMAGE="$runner_image" CANDIDATE_CLUSTER_NAME="$cluster_name" \
    CANDIDATE_CLUSTER_STATE="$state_dir"
  status=$?
  if ((status == 0)); then
  ...
  set -e
  cleanup_cluster "$version_dir" "$cluster_name" "$state_dir" "$runner_image"
  active_version_dir=""
  active_cluster_name=""
  if ((status != 0)); then
